...
<%
  ...
  User staffDetails = (User) session.getAttribute("staffObject");
  ...
  request.setAttribute("path", "_admin-edit-profile.jsp");
%>
<html>
<head>
  <title>Title</title>
  <script>
    ...
    function onLoadStaffInfo(){
      ...
      xhttp.open("POST", "<%=origPath%>/staff/get?userid=<%=userid%>&path=_admin-edit-profile.jsp", true);
      xhttp.send();
    }
  </script>
</head>
...
<div class="ui middle aligned center aligned grid" style="margin-top: 20px">
  <div class="fields">
    <div class="field">
      <form submit='<%=origPath%>/staff/get?userid=<%=userid%>&path=_admin-edit-profile.jsp" method=\"POST\">
        <input class="ui button" type="submit" value="Update Profile"/>
      </form>
    </div>
    <div class="field">
      <form class="ui form" action="<%= request.getContextPath() %>/pages/admin-update?userid=<%=userid%>&path=_admin-edit-profile.jsp" method="POST">
