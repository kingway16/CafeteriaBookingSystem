...
<%
    ...
    request.setAttribute("path", "_customer-profile.jsp");
%>
<html>
<head>
    <title>Title</title>
    <script>
        ...
        function onLoadStaffInfo(){
            ...
            xhttp.open("POST", "<%=origPath%>/customer/get?userid=<%=userid%>&path=_customer-profile.jsp", true);
            xhttp.send();
        }
    </script>
</head>
...
<div class="ui middle aligned center aligned grid" style="margin-top: 20px">
    <div class="fields">
        <div class="field">
            <form class="ui form" action="<%= request.getContextPath() %>/user/update?userid=<%=userid%>&path=_staff-profile.jsp" method="POST">
